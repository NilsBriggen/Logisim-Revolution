...
tasks.register("genBuildInfo") {
  ...
  // Source files need not change when HEAD/branch changes (for example after a commit).
  val gitRevision = if (File("${projectDir}/.git").exists()) providers.exec {
    commandLine("git", "-C", projectDir, "rev-parse", "HEAD")
  }.standardOutput.asText.map { it.trim() } else providers.provider { "source-archive" }
...
tasks {
  ...
  test {
    ...
    doFirst {
      val isolated = Files.createTempDirectory("logisim-tests-").toFile()
      systemProperty("user.home", File(isolated, "home").apply { mkdirs() }.path)
      systemProperty("java.util.prefs.userRoot", File(isolated, "prefs").apply { mkdirs() }.path)
      systemProperty("java.util.prefs.systemRoot", File(isolated, "system-prefs").apply { mkdirs() }.path)
    }
//    testLogging {
...
/**
 * Renders the main window to a PNG for reviewing the interface.
 *
...
tasks.register<JavaExec>("uiSnapshot") {
  group = "verification"
  description = "Renders the main window to a PNG."
  classpath = sourceSets["test"].runtimeClasspath
  mainClass.set("com.cburch.logisim.gui.UiSnapshot")
  systemProperty("snapshot.theme", project.findProperty("theme") ?: "dark")
  systemProperty("snapshot.out", project.findProperty("out") ?: "build/ui-snapshot.png")
  systemProperty("snapshot.width", project.findProperty("width") ?: "1440")
  systemProperty("snapshot.height", project.findProperty("height") ?: "900")
...
tasks.register<JavaExec>("optionsProbe") {
  group = "verification"
  classpath = sourceSets["test"].runtimeClasspath
  mainClass.set("com.cburch.logisim.gui.OptionsProbe")
  args(project.findProperty("file")?.toString() ?: "")
}

tasks.register<JavaExec>("scaleProbe") {
  group = "verification"
  classpath = sourceSets["test"].runtimeClasspath
  mainClass.set("com.cburch.logisim.gui.ScaleProbe")
}

tasks.register<JavaExec>("themeProbe") {
  group = "verification"
  classpath = sourceSets["test"].runtimeClasspath
  mainClass.set("com.cburch.logisim.gui.theme.ThemeProbe")
