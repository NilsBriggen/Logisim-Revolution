...
class TableTab extends AnalyzerTab implements Entry.EntryChangedListener {
  ...
  private final JButton zoomIn = new JButton(AppIcons.get(AppIcons.Id.ZOOM_IN, 14));
  private final JButton zoomOut = new JButton(AppIcons.get(AppIcons.Id.ZOOM_OUT, 14));
  ...
  private void updateScale() {
    ...
    // Size is computed above from the zoom factor, so derive it explicitly rather than letting
    // UiFonts apply the interface scale a second time.
    final var myFont = UiFonts.body().deriveFont(Font.PLAIN, scaledSize);
  ...
  private class TableBody extends JPanel {
    ...
    public void paintComponent(Graphics g, boolean printView, int canvasWidth, int canvasHeight) {
      ...
      g.setColor(printView ? Color.BLACK : Tokens.divider());
