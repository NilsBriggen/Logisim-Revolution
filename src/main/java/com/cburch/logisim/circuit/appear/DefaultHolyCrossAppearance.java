...
public class DefaultHolyCrossAppearance {
  ...
  public static List<CanvasObject> build(Collection<Instance> pins, String name) {
    ...
    rect.setValue(DrawAttr.STROKE_COLOR, CanvasStyle.componentColor());
    ...
    if (name != null && name.length() > 0) {
      ...
      label.getLabel().setColor(CanvasStyle.componentColor());
      ret.add(label);
    }
  ...
  private static void placePins(
      ...
      boolean isLeftSide) {
    int hAlign;
    final var color = CanvasStyle.componentColor();
    int ldx;
    for (final var pin : pins) {
